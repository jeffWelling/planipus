...
| MCP protocol adapter | official `@modelcontextprotocol/sdk` 1.29.0 + Zod 4.1.12 | MIT | **adopt/integrate** | stdio transport, protocol server primitives, strict tool schemas; original API-only Planipus mapping | No direct DB/provider access; no remote HTTP transport; apply off by default; track transitive Hono advisory |
...
| Model Context Protocol | official TypeScript SDK 1.29.0 | Stdio-only adapter over the authoritative Planipus HTTPS/loopback API; dedicated scoped tokens; never expose remote transport without a new auth/security ADR |
