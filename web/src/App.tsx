...
function OverviewScreen({ data, notices, onConnect, onCreate, runAction }: { data: Overview; notices: SyncNotice[]; onConnect: () => void; onCreate: () => void; runAction: ActionRunner }) {
...
export function App() {
  ...
  function refresh() {
    ...
    void Promise.all([api.overview(), api.capabilities(), api.notices().catch(() => [])])
      .then(([value, available, openNotices]) => {
        setOverview(value);
